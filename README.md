...
- React-Query was used to due to it's simplicity
...
- I decided to keep ChildItem as a single component, but would probably split up as soon it becomes bigger
...
- In case an outdated pickup time is send to the server the error is (for now) handled via an alert()

## Next Steps
- Improve the design by adding responsive styling and better visual hierarchy for the dropdowns and buttons
- Add unit and integration tests for core components like ChildItem and hooks
- Use modal to handle error more user friendly
- Add localization/translations
- Add change pickup time feature
- Add textinput for notes ('Today Karla is being picked up by her grandmother.')
